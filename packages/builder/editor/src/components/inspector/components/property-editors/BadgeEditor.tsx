...
import type { BadgeProps } from '@repo/builder/renderer';
import { badgeVariantsOptions } from '@repo/builder/renderer';
...
export const BadgeEditor = ({ block, onUpdate }: PropertyEditorProps) => {
  ...
  const colorOptions = Object.keys(badgeVariantsOptions.variants.color).map(
    (key) => ({
      value: key,
  ...
  const sizeOptions = Object.keys(badgeVariantsOptions.variants.size).map(
    (key) => ({
      value: key,
    ...
    <div className="space-y-3">
      ...
      <div>
        ...
        <select
          ...
          onChange={(e) =>
            updateProps({ color: e.target.value as BadgeProps['color'] })
          }
          className="w-full px-2 py-1.5 text-sm border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500"
        >
      ...
      <div>
        ...
        <select
          value={props.size || 'md'}
          onChange={(e) =>
            updateProps({ size: e.target.value as BadgeProps['size'] })
          }
          className="w-full px-2 py-1.5 text-sm border border-gray-300 rounded focus:outline-none focus:ring-2 focus:ring-blue-500"
        >
